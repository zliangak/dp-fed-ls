...
import sys
sys.path.append('../')
from dp import autodp

from dp.autodp import utils, rdp_bank
from dp.autodp.privacy_calibrator import subsample_epsdelta
import scipy
import math
        ...
        if (func, prob) in self.idxhash:
            ...
            def subsample_func_int(x):
                ...
                if mm <= self.alphas[-1]: # compute the bound exactly. Requires book keeping of O(x^2)
                    ...
                    moments = [np.minimum(np.minimum(j * np.log(np.exp(func(np.inf))-1)
